...
void _to_lu(double *matrix_r, double *inv_matrix, double *rows_getted, int matrix_length, int matrix_rows,
            MPI_Datatype mpi_row, int rank, int size);

void _lu_to_identity(double *matrix_r, double *inv_matrix, double *rows_getted, int matrix_length,
                     MPI_Datatype mpi_row, int rank, int size);
...
void mul_matrix(const double *left, const double *right, double *tmp, size_t matrix_len) {
    if (!tmp) {
        return;
    }
    ...
    for (size_t i = 0; i < matrix_len; ++i) {
        for (size_t k = 0; k < matrix_len; ++k) {
            for (size_t j = 0; j < matrix_len; ++j) {
                tmp[i * matrix_len + j] += left[i * matrix_len + k] * right[k * matrix_len + j];
            }
        }
    }
}
...
void invert_matrix(double *matrix, double *invert_matrix, int matrix_length, int rank, int size) {
    ...
    MPI_Type_contiguous(matrix_length, MPI_DOUBLE, &mpi_row);
    {
        ...
        MPI_Type_vector(max_matrix_rows, matrix_length, matrix_length * size, MPI_DOUBLE, &mpi_row_tmp);
        MPI_Type_create_resized(mpi_row_tmp, 0, sizeof(double) * matrix_length, &mpi_row_sh);
...
//        MPI_Scatter(matrix + matrix_length * size * i, 1, mpi_row,
//                    matrix_r + matrix_length * i, 1, mpi_row,
//                    0, MPI_COMM_WORLD);
//    }
...
//print
    ...
    _to_lu(matrix_r, inv_matrix, rows_getted, matrix_length, matrix_rows, mpi_row, rank, size);
    _lu_to_identity(matrix_r, inv_matrix, rows_getted, matrix_length, mpi_row, rank, size);
    ...
    for (int i = 0; i < max_matrix_rows; i++) {
        ...
        MPI_Gather(inv_matrix + matrix_length * i, 1, mpi_row,
                   invert_matrix + matrix_length * i * size, 1, mpi_row,
                   0, MPI_COMM_WORLD);
    }
...
void _lu_to_identity(double *matrix_r, double *inv_matrix, double *rows_getted, int matrix_length,
    ...
    for (ssize_t i = matrix_length - 1; i > 0; i--) {
        ...
        for (size_t j = rank; j < i; j += size) {
            double mul = matrix_r[(j / size) * matrix_length + i];

            add_rows(matrix_r + (j / size) * matrix_length, row, matrix_length, -mul);
            add_rows(inv_matrix + (j / size) * matrix_length, irow, matrix_length, -mul);
        }
...
void invert_sq_matrix(int matrixsize, double matrix[][matrixsize], double inv_matrix[][matrixsize]) {

    for (size_t row = 0; row < matrixsize; row++) {
        for (size_t col = 0; col < matrixsize; col++) {
            matrix[row][col] = ((double) rand()) / INT_MAX  * 100 - 50;
        }
    }
...
//    printf("====\n\n\n\n");
    ...
    for (size_t i = 0; i < matrixsize; i++) {
        ...
        for (size_t j = i + 1; j < matrixsize; j++) {
            ...
            add_rows(matrix[j], matrix[i], matrixsize, -mul);
            add_rows(inv_matrix[j], inv_matrix[i], matrixsize, -mul);
        }
    }
...
void measure_metrics(int rank, int size, int matrix_length) {
    ...
    if (rank == 0) {
        ...
        for (size_t row = 0; row < matrix_length; row++) {
            for (size_t col = 0; col < matrix_length; col++) {
                matrix[row * matrix_length + col] = ((double) rand()) / INT_MAX * 100 - 50;
            }
        }
        inv_matrix = calloc(sizeof(*matrix), matrix_length * size * max_matrix_rows);
    ...
    if (size == 1) {
        for (int i = 0; i < repeats; i++) {
            invert_sq_matrix(matrix_length, matrix, inv_matrix);
        }
    } else {
        for (int i = 0; i < repeats; i++) {
    ...
    if (rank == 0) {
        ...
        printf("| %10lf | %4d | %11g | %2d |\n", time / repeats, matrix_length, err, size);
        free(matrix);
    }
}
